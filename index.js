...
function isAlmostSame(num1, num2) {
  return num1 - num2 <= Number.EPSILON ? true : false;
...
function isSafeInteger(num) {
  return (Number.MIN_SAFE_INTEGER <= num && num <= Number.MAX_SAFE_INTEGER) ? true : false;
...
function isUnsafeInteger(num) {
  return (!isSafeInteger()) ? true : false;
...
function isTooLarge(num) {
  ...
  return num > Number.MAX_VALUE ? true : false;
}
...
function isAlmostZero(num) {
  return (num > 0 && num < Number.MIN_VALUE) ? true : false;
  // Перевіряємо, чи є число більше 0, але все ще менше за Number.MIN_VALUE
}

console.log("Завдання 6 ====================================");
console.log(
  "isAlmostZero(Number.MIN_VALUE / 2)", // тут є помилка чисто з математики. Якщо будь яке від'ємне число поділити на число більше за 1 то воно не стане меньшим за початкове, а навпаки стане більшим (-10 / 2 = -5 і -5>-10)
...
function checkIsSafeInteger(num) {
  ...
  return Number.isSafeInteger(num) ? true : false;
}
